...
def index():
    ...
    history = db.execute("SELECT record_date, height, weight, bmi FROM records WHERE user_id = ?", session["user_id"])
...
def family():
    ...
    for user_id in user_ids:
        ...
        history = db.execute("SELECT height, weight, bmi, record_date FROM records WHERE user_id = ? ORDER BY record_date DESC", user_id)
